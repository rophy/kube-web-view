...
async def logs_authorized(cluster, namespace: str, resource, session) -> bool:
    ...
    logger.info(
        f"Checking log access for user {user_email} to {resource.kind}/{resource.name} in {namespace}"
    )
    ...
    if "logs-viewer" not in user_groups and "admin" not in user_groups:
        logger.warning(
            f"User {user_email} lacks 'logs-viewer' or 'admin' group for {resource.name}"
        )
        return False
    ...
    if namespace and namespace.startswith("prod-"):
        if "prod-admin" not in user_groups and "admin" not in user_groups:
            logger.warning(
                f"User {user_email} cannot view prod logs for {resource.name} in {namespace}"
            )
            return False
    ...
    if labels.get("confidential") == "true":
        if "security-team" not in user_groups and "admin" not in user_groups:
            logger.warning(
                f"User {user_email} cannot view confidential logs for {resource.name}"
            )
            return False
    ...
    if security_level in ["high", "critical"]:
        ...
        if not any(
            group in user_groups for group in required_groups.get(security_level, [])
        ):
            logger.warning(
                f"User {user_email} lacks required groups for {security_level} security level"
            )
            return False
    ...
    if namespace in namespace_teams:
        ...
        if (
            not any(team in user_groups for team in allowed_teams)
            and "admin" not in user_groups
        ):
            logger.warning(
                f"User {user_email} not authorized for namespace {namespace}"
            )
            return False
        ...
        async with http_session.get(
            ...
            if resp.status == 200:
                ...
                if email_domain not in allowed_domains:
                    logger.warning(
                        f"User {session['user_email']} from unauthorized domain {email_domain}"
                    )
                    return False
